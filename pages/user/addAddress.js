...
Page({
  ...
  addAddress: function(e) {
    ...
    if (e.detail.value.receiver == '') {
      ...
      //请求服务器
      $.http({
        url: wx.getStorageSync('domain') + '/api/user/userAddress?receiver=' + e.detail.value.receiver + '&mobile=' + e.detail.value.mobile + '&address=' + that.data.region + '&addressDetail=' + e.detail.value.addressDetail + '&defaultAddress=' + that.data.first,
        method: 'POST'
      }).then(res => {
        wx.showToast({
          title: '信息提交成功',
